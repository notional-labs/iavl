...
func TestLoadVersionForOverwritingCase2(t *testing.T) {
	...
	tree, _ := NewMutableTreeWithOpts(db.NewMemDB(), 0, nil, false, DefaultFlushThreshold)
...
func TestLoadVersionForOverwritingCase3(t *testing.T) {
	...
	tree, err := NewMutableTreeWithOpts(db.NewMemDB(), 0, nil, false, DefaultFlushThreshold)
	require.NoError(err)
...
func TestNodeCacheStatisic(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(sub *testing.T) {
			...
			db, err := db.NewDB("test", db.MemDBBackend, "")
			require.NoError(t, err)
			mt, err := NewMutableTreeWithOpts(db, tc.cacheSize, opts, false, DefaultFlushThreshold)
			require.NoError(t, err)
