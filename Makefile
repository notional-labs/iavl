...
bench:
	cd benchmarks && \
		...
		go test $(LDFLAGS) -tags cleveldb,rocksdb,boltdb,badgerdb,pebbledb -bench=Small . && \
		go test $(LDFLAGS) -tags cleveldb,rocksdb,boltdb,badgerdb,pebbledb -bench=Medium . && \
...
fullbench:
	cd benchmarks && \
		...
		go test $(LDFLAGS) -tags cleveldb,rocksdb,boltdb,badgerdb,pebbledb -bench=Small . && \
		go test $(LDFLAGS) -tags cleveldb,rocksdb,boltdb,badgerdb,pebbledb -bench=Medium . && \
		go test $(LDFLAGS) -tags cleveldb,rocksdb,boltdb,badgerdb,pebbledb -timeout=30m -bench=Large . && \
